...
[@@@ocaml.text " {1 Payload construction and extraction} "]
...
[@@@ocaml.text " {1 Location management} "]
...
[@@@ocaml.text " {1 Constructing function application} "]
...
[@@@ocaml.text " {1 Generic signature for visitable nodes} "]
type 'a iter = 'a -> unit
type 'a map = 'a -> 'a
module type VisitableS  =
  sig
    type t
...
sig
  ...
  sig
    val functor_ :
      functor_parameter -> Types.module_type -> Types.module_type
  ...
  val get_desc : Types.type_expr -> Types.type_desc[@@ocaml.doc
                                                     " Getter for [type_expr] introduced in OCaml 4.14 "]
  val get_level : Types.type_expr -> int[@@ocaml.doc
                                          " Getter for [type_expr] introduced in OCaml 4.14 "]
  val get_scope : Types.type_expr -> int[@@ocaml.doc
                                          " Getter for [type_expr] introduced in OCaml 4.14 "]
  val get_id : Types.type_expr -> int[@@ocaml.doc
                                       " Getter for [type_expr] introduced in OCaml 4.14 "]
end
[@@@ocaml.text " {1 Generic signature for expressions and patterns} "]
module type ValueS  =
  sig
    include ExtensibleS
    ...
    val list : ?attrs:Ppxlib.attributes -> ?prefix:Longident.t -> t list -> t
  end
module Pat :
sig
...
sig
  ...
  val chop :
    string ->
      Ppxlib.attributes -> (Ppxlib.attribute * Ppxlib.attributes) option
    ...
    ?loc:Location.t ->
      ?attrs:Ppxlib.attributes ->
        string option Location.loc ->
          Ppxlib.module_type -> Ppxlib.module_declaration
end
module Mb :
sig
  val mk :
    ?loc:Location.t ->
      ?attrs:Ppxlib.attributes ->
        string option Location.loc ->
          Ppxlib.module_expr -> Ppxlib.module_binding
end
...
[@@@ocaml.text " {1 Open} "]
module Opn : sig type 'a t = 'a Ppxlib.open_infos end
...
sig
  ...
  val ident :
    ?loc:Location.t ->
      ?attrs:Ppxlib.attributes -> Longident.t -> Ppxlib.expression
  val ident_of_str :
    ?attrs:Ppxlib.attributes -> Ast_helper.str -> Ppxlib.expression
  val send :
    ?loc:Location.t ->
      ?attrs:Ppxlib.attributes ->
        Ppxlib.expression -> Ast_helper.str -> Ppxlib.expression
  val newtype :
    ?loc:Location.t ->
      ?attrs:Ppxlib.attributes ->
        Ast_helper.str -> Ppxlib.expression -> Ppxlib.expression
  val open_ :
    ?loc:Location.t ->
      ?attrs:Ppxlib.attributes ->
        Ppxlib.module_expr Opn.t -> Ppxlib.expression -> Ppxlib.expression
  val destruct_open :
    Ppxlib.expression ->
...
sig
  type desc =
    | Rtag of Ppxlib.Asttypes.label Location.loc * bool * Ppxlib.core_type
    list 
    ...
    ?loc:Location.t ->
      ?attrs:Ppxlib.attributes ->
        Ppxlib.Asttypes.label Location.loc ->
          bool -> Ppxlib.core_type list -> Ppxlib.row_field
  val inherit_ :
    ?loc:Location.t ->
      ?attrs:Ppxlib.attributes -> Ppxlib.core_type -> Ppxlib.row_field
end[@@ocaml.doc
...
sig
  ...
  val inherit_ :
    ?loc:Location.t -> ?attrs:Ppxlib.attributes -> Ppxlib.core_type -> t
end
[@@@ocaml.text " {1 With constraint} "]
module With :
sig
  val typesubst :
    ?t:Ast_helper.lid -> Ppxlib.type_declaration -> Ppxlib.with_constraint
  val destruct_typesubst :
    Ppxlib.with_constraint ->
      (Ast_helper.lid * Ppxlib.type_declaration) option
  val modsubst : Ast_helper.lid -> Ast_helper.lid -> Ppxlib.with_constraint
...
sig
  ...
  val empty : 'a t[@@ocaml.doc " The empty accumulator. "]
  val add : 'a -> 'a t -> (int * 'a t)[@@ocaml.doc
  ...
  val to_array : 'a t -> 'a array[@@ocaml.doc
                                   " [to_array a] returns the array containing all the elements of [a]. "]
end[@@ocaml.doc " Indexed accumulator to build an array. "]
